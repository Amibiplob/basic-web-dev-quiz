...
const Quiz = () => {
    ...
    const answer = (ans) => {
        ...
        for (const correctAns of questions) {
            if (ans === correctAns.correctAnswer) {
            ...
            <h1 className='text-4xl text-right'>total quiz  {total}</h1>
            {
                questions.map(data => (
                    <div key={data.id}>
                        ...
                        <div className='bg-gray-300 grid grid-cols-1 lg:grid-cols-2 items-center'>
                            {data.options.map(data => (
                                <div className="p-3" key={data}>
                                    <label className="label cursor-pointer bg-slate-200 rounded-xl pl-2  hover:bg-slate-600" onClick={() => answer(data)}>
                                        <input type="radio" name="radio-6" className="radio checked:bg-blue-500" />
                                        <span className="label-text flex-1 ml-3  hover:text-white">{data}</span>
                                    </label>
                                </div>
                            ))
                            }
